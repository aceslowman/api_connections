...
class OSCDatagram
{
    ...
    /** Returns a semi-human readable representation of the binary data.
      ...
      */
    public function get_human_readable($hex_only = false)
    {
        $bin = $this->get_binary();
        $hex = "";
    ...
    /** Pack data into $this->bin as 4-byte aligned, network-byte order.
      */
    public function pack_data($data, $type_hint)
    {
        $bin = "";
        switch ($type_hint) {
            case "T":
    ...
    /** Utility to generate padding for strings
      */
    public function get_strpad($str)
    {
        $x = (strlen($str)) % 4;
        if ($x == 0) {
            return '';
...
class OSCMessage extends OSCDatagram
{
    ...
    /** Make a new message - Optionally specify address and arguements.
      ...
      */
    public function OSCMessage($address = null, $args = null)
    {
        if (! is_null($address)) {
    ...
    /** Add an arg to the OSC message.
      ...
      */
    public function add_arg($data, $type_hint = null)
    {
        $this->bin = null;
        if ($type_hint == null) {
    ...
    /** Cast data to type, and add type info to typetags.
      */
    public function set_type($data, $type_tag)
    {
        switch ($type_tag) {
            case "i":
                $this->typetags .= "i";
...
class OSCBundle extends OSCDatagram
{
    ...
    /** Create a new OSCBundle datagram
      ...
      */
    public function OSCBundle($init = null)
    {
        if (is_array($init)) {
            foreach ($init as $d) {
                $this->add_datagram($d);
    ...
    /** Set time tag as whole seconds since July 1, 1970, and fraction of a second.
      ...
      */
    public function set_timetag($timetag_obj)
    {
    ...
    /** Add an OSCDatagram object to a bundle.
      ...
      */
    public function add_datagram($osc_datagram)
    {
        $this->bin = null;
...
class OSCClient
{
    ...
    public function OSCClient($address = null, $port = null)
    {
        $this->address = $address;
        $this->port = $port;
...
class Timetag
{
    public function Timetag($sec = 0, $frac_sec = 1)
    {
        $this->sec = $sec;
...
{
    $c = new OSCClient();
    $c->set_destination("127.0.0.1", 12345);
...
//   $c = new OSCClient();
//   $c->set_destination("192.168.1.101", 7400);
//   $c->send(new OSCMessage("/foo", array(1,2,3)));
